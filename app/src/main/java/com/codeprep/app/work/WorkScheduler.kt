...
class WorkScheduler @Inject constructor(
    ...
    private val heartRefillAlarmScheduler: HeartRefillAlarmScheduler
) {
    ...
    suspend fun syncHeartReminder(userId: String) {
        val workManager = WorkManager.getInstance(context)
        workManager.cancelUniqueWork(WorkConstants.LEGACY_HEART_WORK_NAME).await()
        workManager.cancelUniqueWork(WorkConstants.HEART_REMINDER_WORK_NAME).await()

        val progress = userRepository.getUserProgressOnce(userId)
        val delay = progress?.let { HeartReminderSchedule.delayFor(it) }
        if (delay == null) {
            heartRefillAlarmScheduler.cancel()
            return
        }
        ...
        workManager.enqueueUniqueWork(
            ...
            OneTimeWorkRequestBuilder<HeartWorker>()
                ...
                .setInitialDelay(safeDelay)
                .build()
        ).await()

        heartRefillAlarmScheduler.schedule(
            triggerAtMillis = System.currentTimeMillis() + safeDelay.toMillis(),
            userId = userId
        )
    }
    ...
    suspend fun cancelSessionWorkers() {
        ...
        workManager.cancelUniqueWork(WorkConstants.LEGACY_HEART_WORK_NAME).await()
        workManager.cancelUniqueWork(WorkConstants.HEART_REMINDER_WORK_NAME).await()
        workManager.cancelUniqueWork(WorkConstants.STREAK_WORK_NAME).await()
        workManager.cancelUniqueWork(WorkConstants.SYNC_WORK_NAME).await()
        heartRefillAlarmScheduler.cancel()
    }
}
